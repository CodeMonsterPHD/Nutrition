...
def main():
    ...
    all_data = ReadCsvData('./../data/vfdl/pittv8.csv')
...
def get_anchor(segment,ann,t,phase):
    after_train = []
    train_img_list = os.listdir('./../data/vfdl/train_resize')
    test_img_list = os.listdir('./../data/vfdl/test_resize')
...
def write_json(ls,t,phase,segment):
    with open('./../lib/dataset/vfdl_Bin_number10/'+'VFD_'+phase+'_'+t +'_seg'+str(segment)+'.json', 'wb') as f1:
        f1.truncate()
...
def creat_csv(food_name):
    path = "./../lib/evaluation/data_gt/vfdl/" + food_name + "_test_gt.csv"
